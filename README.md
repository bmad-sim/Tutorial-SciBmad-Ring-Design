...
Start JupyterLab from the project root so the relative paths used by the
tutorial stay valid. Before running a numbered chapter for the first time, open
...
against. Each numbered chapter activates the shared project, but it does not
repeat `Pkg.instantiate()`.

### Running the chapters

After the one-time setup, open the numbered notebooks from the same JupyterLab
session. Each new Julia kernel loads the packages used by that chapter through
its `using` and `import` statements.
